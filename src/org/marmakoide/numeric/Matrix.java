...
public class Matrix {
   ...
   *          no. cols of the matrix
   * @param inOffset
   *          index of the first coefficient of the matrix in the mapped array
   * @param inPitch
   *          number of coefficients to skip, to move from the head of a column
   *          to the next column's head
   * @param inData
   *          the array to map
   */
   ...
   */
  public static Matrix zeros(final int inRows, final int inCols) {
   ...
   * @return a new matrix instance
   */
  public static Matrix ones(final int inRows, final int inCols) {
   ...
   * @return this matrix
   */
  public Matrix set(final int i, final int j, final double inValue) {
   ...
   * @return this matrix
   */
  public Matrix inc(final int i, final int j, final double inValue) {
    this.mData[this._index(i, j)] += inValue;
    return this;
  }
   ...
   * @return this matrix
   */
  public Matrix dec(final int i, final int j, final double inValue) {
    this.mData[this._index(i, j)] -= inValue;
    return this;
  }
  ...
  public Vector row(final int i) {
    return new Vector(this.mCols, this.mOffset + i, this.mPitch, this.mData);
  }
   ...
   */
  public ColWiseMatrixProxy colWise() {
   ...
   */
  public RowWiseMatrixProxy rowWise() {
    return new RowWiseMatrixProxy(this);
  }

  // --- Filling --------------------------------------------------------------

  public Matrix fillFromValues(final double... inValues) {
  ...
  public Matrix fillAsRandNorm(final Random inRandom) {
   ...
   * @return this matrix
   */
  public Matrix scaledAddCross(final double inValue, final Vector inVector) {
    final int uBegInc = this.mPitch;
  ...
  public Vector dot(final Vector inVector) {
    final Vector lRet = new Vector(this.mRows);
    this._dot(inVector, lRet);
    return lRet;
  }

  public Vector dot(final Vector inVector, final Vector outResult) {
    this._dot(inVector, outResult);
    return outResult;
  }

  private void _dot(final Vector inVector, final Vector outResult) {
    final int vBeg = inVector.getOffset();
    final int vInc = inVector.getStride();
  ...
  public Matrix dot(final Matrix inMatrix) {
    final Matrix lRet = new Matrix(this.mRows, inMatrix.mCols);
    this._dot(inMatrix, lRet);
    return lRet;
  }

  public Matrix dot(final Matrix inMatrix, final Matrix outResult) {
    this._dot(inMatrix, outResult);
    return outResult;
  }

  private void _dot(final Matrix inMatrix, final Matrix outResult) {
  ...
  public final Matrix fill(final double inValue) {
    if (this.mPitch == this.mRows) {
      Arrays
          .fill(this.mData, this.mOffset, this.mOffset + this.mSize, inValue);
    } else {
      final int uBegInc = this.mPitch;
   ...
   * @return this matrix
   */
  public final Matrix scaledAdd(final double inValue, final Matrix inMatrix) {
   ...
   * @return this matrix
   */
  public final Matrix scaledCopy(final double inValue, final Matrix inMatrix) {
   ...
   * @return this matrix
   */
  public final Matrix invScaledCopy(final double inValue, final Matrix inMatrix) {
   ...
   *          whether the reduction is performed col-wise or row-wise
   * @param outResult
   *          the vector in which the col-wise or row-wise reduction will be
   *          stored
   ...
   * @return the vector given as input
   */
  public Vector squareSum(final boolean inColWise, final Vector outResult) {
    return this.reduce(new SquareSum(), inColWise, outResult);
  }

  /**
   * Computes the col-wise or row-wise sum of the coefficient's absolute sum
   * 
   * @param inColWise
   *          computes col-wise or row-wise
   * @return a new vector instance in which the col-wise or row-wise sum will
  ...
  /**
   * Computes the col-wise or row-wise sum of the coefficient's absolute value
   * 
   * @param inColWise
   *          computes col-wise or row-wise
   * @param outResult
